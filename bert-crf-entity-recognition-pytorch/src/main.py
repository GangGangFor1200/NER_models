...
class Manager():
    def __init__(self, mode, config_path, ckpt_name=None):
        ...
        assert self.config['turn_type'] == 'single' or self.config['turn_type'] == 'multi', print(
            "Please specify a correct turn type, either 'single' or 'multi'.")
        ...
        if self.config['turn_type'] == 'multi':
            assert self.config['sentence_embedding'] == 'cls' or self.config['sentence_embedding'] == 'max' or self.config['sentence_embedding'] == 'mean', \
                print(
                    "Please specify a correct sentence embedding method among these three, 'cls', 'max' and 'mean'.")
            print(
                f"Sentence embedding policy is {self.config['sentence_embedding']}.")
        ...
        if ckpt_name is not None:
            assert os.path.exists(
                f"{self.config['ckpt_dir']}/{ckpt_name}"), f"There is no checkpoint named {ckpt_name}."
        ...
        if mode == 'train':
            ...
            train_set = CustomDataset(
                f"{full_dir}/{self.config['train_name']}", self.tokenizer, self.config, class_dict)
            valid_set = CustomDataset(
                f"{full_dir}/{self.config['valid_name']}", self.tokenizer, self.config, class_dict)
            self.train_loader = DataLoader(
                train_set, shuffle=True, batch_size=self.config['batch_size'])
            self.valid_loader = DataLoader(
                valid_set, shuffle=True, batch_size=self.config['batch_size'])
        elif mode == 'test':
            # Load test dataset
            test_set = CustomDataset(
                f"{full_dir}/{self.config['test_name']}", self.tokenizer, self.config, class_dict)
            self.test_loader = DataLoader(
                test_set, shuffle=True, batch_size=self.config['batch_size'])
    ...
    def train(self):
        ...
        for epoch in range(1, self.config['num_epochs']+1):
            ...
            for i, batch in enumerate(tqdm(self.train_loader)):
                ...
                pad_id = self.tokenizer._convert_token_to_id(
                    self.config['pad_token'])

                if self.config['turn_type'] == 'single':
                    batch_x, batch_y = batch_x.to(
                        self.config['device']), batch_y.to(self.config['device'])
                    log_likelihood, tag_seq = self.model(
                        batch_x, pad_id, batch_y)  # (), (B, L)
                elif self.config['turn_type'] == 'multi':
                    batch_x, batch_y, batch_times = \
                        batch_x.to(self.config['device']), batch_y.to(
                            self.config['device']), batch_times.to(self.config['device'])
                    log_likelihood, tag_seq = self.model(
                        batch_x, pad_id, batch_y, times=batch_times)  # (), (B, L)
                ...
                true_list = list(chain.from_iterable(
                    [sublist[:batch_lens.tolist()[b]] for b, sublist in enumerate(batch_y.tolist())]))
                train_true += true_list

                assert len(pred_list) == len(
                    true_list), "Please check if the length of predictions and that of true labels are identical."

            mean_train_loss = np.mean(train_losses)
            train_acc = accuracy_score(train_true, train_pred)
            train_f1 = f1_score(train_true, train_pred, average='weighted')
            print(
                f"Train loss: {mean_train_loss} || Train accuracy: {train_acc} || Train F1 score: {train_f1}")
            ...
            print(
                f"Validation loss: {valid_loss} || Validation accuracy: {valid_acc} || Current validation F1 score: {valid_f1}")
    ...
    def test(self):
        ...
        print("#################### Test results ####################")
        print(
            f"Test loss: {mean_test_loss} || Test accuracy: {test_acc} || Test F1 score: {test_f1}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--config_path', required=True,
                        type=str, help="The path to configuration file.")
    ...
    assert args.mode == 'train' or args.mode == 'test', print(
        "Please specify a correct mode name, 'train' or 'test'.")
