    ...
    for i, line in enumerate(tqdm(lines)):
        if line.strip() != config['dialogue_split']:
            ...
            if len(comps) > 2:
                ...
                tokens, entity_tags = find_entities(
                    tokens, entities, entity_tags, tokenizer)
...
def find_entities(tokens, entities, entity_tags, tokenizer):
    ...
    for entity in entity_list:
        ...
        entity_tags, checked = find_sublist(
            tokens, entity_tokens, entity_tag, entity_tags, checked)
        ...
        if e == sub[0] and not checked[i]:
            ...
            if cand == sub:
                ...
                if len(sub) > 1:
                    ...
                    entity_tags = [f'I-{tag}' if cur_tag == 'O' and (j > i and j < i+len(
                        sub)) else cur_tag for j, cur_tag in enumerate(entity_tags)]
...
def save_text(full_dir, dialogues, dialogue_labels):
    ...
    with open(f"{full_dir}/inputs.txt", 'w', encoding='UTF8') as f:
    ...
    with open(f"{full_dir}/labels.txt", 'w', encoding='UTF8') as f:
...
def save_ids(full_dir, dialogues, dialogue_labels):
    ...
    with open(f"{full_dir}/inputs.ids", 'w') as f:
        for d, dialogue_tokens in enumerate(tqdm(dialogues)):
            ...
            for t, tokens in enumerate(dialogue_tokens):
                speaker = tokens[0]
                sent = [str(tokenizer._convert_token_to_id(token))
    ...
    with open(f"{full_dir}/labels.ids", 'w', encoding='UTF8') as f:
        for d, dialogue_tags in enumerate(tqdm(dialogue_labels)):
            for t, tags in enumerate(dialogue_tags):
...
if __name__ == '__main__':
    ...
    for file in file_list:
        if os.path.isfile(f"{config['data_dir']}/{config['original_dir']}/{file}"):
            ...
            tokens, tags = load_file(
                f"{config['data_dir']}/{config['original_dir']}/{file}", tokenizer)
            train_tokens, train_tags = tokens[:int(
                len(tokens)*config['train_frac'])], tags[:int(len(tags)*config['train_frac'])]
            remained_tokens, remained_tags = tokens[int(
                len(tokens)*config['train_frac']):], tags[int(len(tags)*config['train_frac']):]

            f = config['valid_frac'] / (1.0-config['train_frac'])
            valid_tokens, valid_tags = remained_tokens[:int(
                len(remained_tokens)*f)], remained_tags[:int(len(remained_tags)*f)]
            test_tokens, test_tags = remained_tokens[int(
                len(remained_tokens)*f):], remained_tags[int(len(remained_tags)*f):]
    ...
    save_text(f"{full_dir}/{config['train_name']}",
              total_train_tokens, total_train_tags)
    save_ids(f"{full_dir}/{config['train_name']}",
             total_train_tokens, total_train_tags)
    save_text(f"{full_dir}/{config['valid_name']}",
              total_valid_tokens, total_valid_tags)
    save_ids(f"{full_dir}/{config['valid_name']}",
             total_valid_tokens, total_valid_tags)
    save_text(f"{full_dir}/{config['test_name']}",
              total_test_tokens, total_test_tags)
    save_ids(f"{full_dir}/{config['test_name']}",
             total_test_tokens, total_test_tags)
